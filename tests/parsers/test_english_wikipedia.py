...
class TestEnglishWikipediaUSASDefaults:
    """
    Verifies that EnglishWikipediaUSAS pre-fills dataset_name and language
    with the correct English corpus defaults. Full parsing behaviour is
    covered by TestWikipediaUSAS in test_wikipedia.py.
    """

    @pytest.fixture
    def get_test_directory(self, get_test_data_directory: Path) -> Path:  # noqa: F811
        return get_test_data_directory / "parsers" / "english_wikipedia"
    ...
    def test_is_wikipedia_usas_subclass(self) -> None:
        """EnglishWikipediaUSAS must be a subclass of WikipediaUSAS."""
        assert issubclass(EnglishWikipediaUSAS, WikipediaUSAS)

    def test_default_dataset_name(self, get_wikipedia_test_directory: Path) -> None:
        """Default dataset_name is 'Medical Wikipedia'."""
        dataset = EnglishWikipediaUSAS.parse(
    ...
    def test_parse_full_dataset(self, get_test_directory: Path) -> None:
        """Parse the full english.csv corpus and verify sentence/token counts."""
        data_file = get_test_directory / "english_wikipedia_corpus.csv"
        ...
        assert token_count == 4036
        assert semantic_tag_count == 3505
